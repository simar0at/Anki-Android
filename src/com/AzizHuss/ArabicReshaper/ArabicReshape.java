...
public class ArabicReshape {
	...
	static final char NUMBERS_CHAR=0x000B;
    ...
    private static final char allchar[][] = {
        ...
        {0x064B, 0x000C, 0x064B, 0xFE70, 0xFE71, 0xFE70}, // fathatan
        {0x064C, 0x000C, 0x064C, 0xFE72, 0xFE72, 0xFE72}, // dammatan
        {0x064D, 0x000C, 0x064D, 0xFE74, 0xFE74, 0xFE74}, // kasratan
        {0x064E, 0x000A, 0x064E, 0xFE76, 0xFE77, 0xFE76}, // fatha
        {0x064F, 0x000A, 0x064F, 0xFE78, 0xFE79, 0xFE78}, // damma
        {0x0650, 0x000A, 0x0650, 0xFE7A, 0xFE7B, 0xFE7A}, // kasra
        {0x0651, 0x000A, 0x0651, 0xFE7C, 0xFE7D, 0xFE7C}, // tashdeed
        {0x0652, 0x000A, 0x0652, 0xFE7E, 0xFE7F, 0xFE7E}, // sukun
        ...
        {0x0670, 0x000A, 0x0670, 0x0, 0x0, 0x0}, // superscript alef
    	...
    	{0x06BD, 0x000F, 0x0, 0x0, 0x0, 0x0},
         ...
         private static Pattern HTMLStuff = null;
		
	     public static String reshape(String Str){
	    	 String Temp="   "+Str+"   ";
	    	 char pre,pre_pre,pre_pre_pre,at,post,post_post,post_post_post;
	    	 StringBuilder  reshapedString=new StringBuilder() ;
	    	 reshapedString.append("\u0640"); //enforce right-to-left even for the very last character which could be a tashkeel
	    	 int i=0;
	    	 int len=Str.length();
	    	 
	    	 while (i<len){
	    		pre_pre_pre=Temp.charAt(i);
	 			pre_pre=Temp.charAt(i+1);
	 			pre=Temp.charAt(i+2);
				at=Temp.charAt(i+3);
				post=Temp.charAt(i+4);
				post_post=Temp.charAt(i+5);
				post_post_post=Temp.charAt(i+6);
				

				int what_case_pre_pre_pre=getCase(pre_pre_pre);
				int what_case_pre_pre=getCase(pre_pre);
				...
				int what_case_post_post_post=getCase(post_post_post);

				int pre_step=0;
				//tashkeel have no influence on what sort of character we have to choose.
				if (what_case_pre==TASHKEEL){
					pre=pre_pre;
					what_case_pre=getCase(pre);
					//there can be 2 of them (tashdeed)
					if (what_case_pre==TASHKEEL){
						pre=pre_pre_pre;
						what_case_pre=getCase(pre);
					}
				...
				case RIGHT_LEFT_CHAR:
					// simplest case: just another character
					if ((what_case_post&RIGHT_NOLEFT_CHAR_MASK)==RIGHT_NOLEFT_CHAR_MASK){
						...
						if (what_case_post_post==TASHKEEL){
							if ((what_case_post_post_post&RIGHT_NOLEFT_CHAR_MASK)==RIGHT_NOLEFT_CHAR_MASK){
								reshapedString.append(getShape(at,2+pre_step));
								i=i+1;
								continue;								
							} else {
						...
						} else if ((what_case_post_post&RIGHT_NOLEFT_CHAR_MASK)==RIGHT_NOLEFT_CHAR_MASK){
							reshapedString.append(getShape(at,2+pre_step));
							i=i+1;
							continue;							
						} else {
				...
				case RIGHT_LEFT_CHAR_LAM:
                    if ((what_case_post&0x000F)==RIGHT_NOLEFT_CHAR_ALEF){
						...
						if (what_case_post_post==TASHKEEL){
							if ((what_case_post_post&0x000F)==RIGHT_NOLEFT_CHAR_ALEF){
								reshapedString.append(getShape(post_post,pre_step+2));
								i=i+3;		
								continue;
							} else if ((what_case_post_post_post&RIGHT_NOLEFT_CHAR_MASK)==RIGHT_NOLEFT_CHAR_MASK){
								reshapedString.append(getShape(at,2+pre_step));
								i=i+1;
								continue;								
							} else {
						...
						} else if ((what_case_post_post&0x000F)==RIGHT_NOLEFT_CHAR_ALEF){
							reshapedString.append(getShape(post_post,pre_step+2));
							i=i+3;
		...
		//		System.out.println("##letter "+ pre);
	     ...
	     public static String reshape_browser(String Str){
	    	 if (HTMLStuff == null)
	    		 HTMLStuff = Pattern.compile("((?:<.*?>)|(?:&.*?;))");
	    	 // find the HTMLParts and reverse them as they will be reversed again later on.
	    	 Matcher findHtmlParts = HTMLStuff.matcher(Str);
	    	 StringBuffer tmp = new StringBuffer();
	    	 StringBuilder reshapedString = new StringBuilder();
	    	 while (findHtmlParts.find()) {
	    		 findHtmlParts.appendReplacement(tmp, reshapedString.insert(0, findHtmlParts.group(1)).reverse().toString());
	    		 reshapedString.setLength(0);
	    	 }
	    	 findHtmlParts.appendTail(tmp);
             reshapedString.insert(0, reshape(tmp.toString())); 
             // tatweel was added force the end harakat to be on the correct side of the word.
             // Check if we actually did anything useful.
	    	 if (!reshapedString.toString().equals('\u0640' + tmp.toString() + '\u0640'))
	    	 {	    	   
               reshapedString.reverse();
	    	   return reshapedString.toString();
	    	 }
	    	 else
	         // Str were characters we didn't recognize so just return what we got.
	    		 return Str;
	     } 	     
